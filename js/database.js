...
class Database {
    ...
    addUser(userData, role) {
        ...
        const newUser = { id: newId, ...userData, role, status: 'aktif' };
        users.push(newUser);
    ...
    addKendaraan(kendaraanData) {
        ...
        const newKendaraan = { id: newId, ...kendaraanData, status: 'pending' };
    ...
    updateKendaraan(id, updates) {
        ...
        if (index !== -1) {
            kendaraan[index] = { ...kendaraan[index], ...updates };
            localStorage.setItem('siparkir_kendaraan', JSON.stringify(kendaraan));
    ...
    checkOut(transaksiId) {
        ...
        if (index !== -1) {
            ...
            const durasi = Math.floor((waktuKeluar - waktuMasuk) / (1000 * 60)); // dalam menit
            
            transaksi[index].waktuKeluar = waktuKeluar.toISOString();
            transaksi[index].durasi = durasi;
    ...
    updateLaporan(id, updates) {
        ...
        if (index !== -1) {
            laporan[index] = { ...laporan[index], ...updates };
            localStorage.setItem('siparkir_laporan_kehilangan', JSON.stringify(laporan));
